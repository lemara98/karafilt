...
### Fixed
- **Wrong-song lyrics are far rarer.** Four causes fixed: a same-titled song
  by a clearly different artist is no longer accepted when the artist doesn't
  match (unless the recording length corroborates it); lyric lookups now
  resolve in candidate-trust order instead of first-response-wins, so a weak
  title-only match can't beat the exact artist+track match just by returning
  faster; lyrics fetched for one video are never re-served to a different
  video from the cache; and the Genius fallback now requires the artist to
  match too instead of taking any same-titled song.
